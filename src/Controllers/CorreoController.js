...
exports.enviarCorreoyCuerpo = async (req, res) => {
  try {
    ...
    enviarCodigoVerficiacionActivaCuenta(email, codigoVerificacion);
...
//   Usuario.updateOne({ correo: correo }, { $set: { token: token } }).then(
//     (result) => {
//       console.log();
//     }
//   );
...
//   res.status(200).json({ msg: "correo elctronico enviado correctamente" });
// };
...
function enviarCodigoVerficiacionActivaCuenta(email) {
  const mailOptions = {
    from: '"Hello 👋" <tu_correo@example.com>',
    to: email,
    ...
    <p style="font-size: 20px; font-weight: bold; background-color: #f0f0f0; padding: 10px; border-radius: 5px; text-align: center;" id="codigo">${codigoVerificacion}</p>
    
    <div style="text-align: center; margin-top: 20px;">
      <button onclick="copyToken()" style="padding: 10px 20px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer;">Copiar Código</button>
    </div>
